...
def create_experiments_or(model):
    ...
    return  [
                           ...
                           # Measure orientation tuning with full-filed sinusoidal gratins
                           MeasureOrientationTuningFullfield(model,ParameterSet({'num_orientations' : 2,'spatial_frequency':0.8,'temporal_frequency':2,'grating_duration':2*143*7,'contrasts':[100],'num_trials':10})),
...
def create_experiments_stc(model):
    ...
    return  [
                           ...
                           #Size Tuning  
                           MeasureSizeTuning(model,ParameterSet({'num_sizes': 10,'max_size':3.0,'log_spacing' : True,'orientation' : 0,'spatial_frequency' : 0.8,'temporal_frequency' : 2,'grating_duration' : 10*2*143*7,'contrasts' : [5,100],'num_trials' : 1,'with_flat': False})),
            ]
