...
if True:
    ...
    experiment_list =   [
                           ...
                           #MeasureSpontaneousActivity(jens_model,duration=147*7,num_trials=4),
			   
			   #Size tuning
     			   MeasureSizeTuning(jens_model,num_sizes=2,max_size=4.0,orientation=numpy.pi/2,spatial_frequency=0.8,temporal_frequency=2,grating_duration=147*7,contrasts=[20,100],num_trials=1),
...
if False: # PLOTTING
    ...
    lgn_neuron = param_filter_query(data_store,sheet_name="X_ON").get_segments()[0].get_stored_vm_ids()[0]
    
    dsv = param_filter_query(data_store,st_name='FullfieldDriftingSinusoidalGrating')    
    OverviewPlot(dsv,ParameterSet({'sheet_name' : 'X_ON', 'neuron' : lgn_neuron, 'sheet_activity' : {}}),fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
    
    RetinalInputMovie(dsv,ParameterSet({'frame_rate' : 10})).plot()
    import pylab
    pylab.show()

    dsv = param_filter_query(data_store,st_name='FullfieldDriftingSinusoidalGrating')    
    OverviewPlot(dsv,ParameterSet({'sheet_name' : 'V1_Exc_L4', 'neuron' : l4_exc, 'sheet_activity' : activity_plot_param}),fig_param={'dpi' : 100,'figsize': (14,12)}).plot({'Vm_plot.y_lim' : (-67,-56),'Conductance_plot.y_lim' : (0,35.0)})
