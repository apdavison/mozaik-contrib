...
if True:
    ...
    setup_logging()
    data_store = PickledDataStore(load=True,parameters=ParameterSet({'root_directory':'SingleStimulus'}))
    #SingleStimulus
    #NIWEM
...
if (not MPI) or (mpi_comm.rank == MPI_ROOT):
    ...
    if False:
        ...
        TrialAveragedFiringRate(select_result_sheet_query(data_store,'V1_Exc_L4'),ParameterSet({'stimulus_type':"DriftingSinusoidalGratingDisk"})).analyse()
        TrialAveragedFiringRate(select_result_sheet_query(data_store,'V1_Inh_L4'),ParameterSet({'stimulus_type':"DriftingSinusoidalGratingDisk"})).analyse()
        
        #RetinalInputMovie(select_stimuli_type_query(data_store,'DriftingSinusoidalGratingDisk'),ParameterSet({'frame_rate' : 5})).plot()
        
        dsv = analysis_data_structure_stimulus_filter_query(data_store,'DriftingSinusoidalGratingDisk')
        dsv = analysis_data_structure_parameter_filter_query(dsv,analysis_algorithm='TrialAveragedFiringRate')
        ...
        dsv = select_stimuli_type_query(data_store,'DriftingSinusoidalGratingDisk')
    ...
    if False:
        ...
        dsv = analysis_data_structure_stimulus_filter_query(data_store,'FullfieldDriftingSinusoidalGrating')
        ModulationRatio(select_result_sheet_query(dsv,'V1_Exc_L2/3'),ParameterSet({'bin_length' : 10.0 })).analyse()
