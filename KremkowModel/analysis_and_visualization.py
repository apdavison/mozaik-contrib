...
def perform_analysis_and_visualization(data_store):
    ...
    if False:  #ANALYSIS
        ...
        #param_filter_query(data_store,analysis_algorithm='Analog_MeanSTDAndFanoFactor').remove_ads_from_datastore()
        #Analog_MeanSTDAndFanoFactor(data_store,ParameterSet({})).analyse()
        #data_store.save()
        
        TrialToTrialCrossCorrelation(param_filter_query(data_store,sheet_name='V1_Exc_L4',st_orientation=[0,numpy.pi/2]),ParameterSet({'neurons' : [l4_exc], 'bin_length' : 2.0 })).analyse()
        data_store.save()
        
        #TrialVariability(dsv,ParameterSet({'vm': True,  'cond_exc': False, 'cond_inh': False})).analyse()
        
        #GSTA(param_filter_query(data_store,sheet_name='V1_Exc_L4',stimulus_id='InternalStimulus',direct_stimulation_name='Kick'),ParameterSet({'neurons' : [l4_exc], 'length' : 250.0 }),tags=['GSTA']).analyse()
    ...
    if True: # PLOTTING
        ...
        #OverviewPlot(dsv,ParameterSet({'sheet_name' : 'V1_Exc_L4', 'neuron' : l4_exc, 'sheet_activity' : {}}),fig_param={'dpi' : 50,'figsize': (30,12)},plot_file_name="Exc1.png").plot({'Vm_plot.y_lim' : (-67,-56),'Conductance_plot.y_lim' : (0,35.0)})
        
        dsv = param_filter_query(data_store,st_name='FullfieldDriftingSinusoidalGrating')    
        #OverviewPlot(dsv,ParameterSet({'sheet_name' : 'V1_Exc_L4', 'neuron' : l4_exc, 'sheet_activity' : {}}),fig_param={'dpi' : 50,'figsize': (30,12)},plot_file_name="Exc2.png").plot({'Vm_plot.y_lim' : (-67,-56),'Conductance_plot.y_lim' : (0,35.0)})
        ...
        OverviewPlot(data_store,ParameterSet({'sheet_name' : 'X_ON', 'neuron' : sorted(param_filter_query(data_store,sheet_name="X_ON").get_segments()[0].get_stored_esyn_ids())[0], 'sheet_activity' : {}}),fig_param={'dpi' : 50,'figsize': (28,24)},plot_file_name="LGN0On.png").plot()
        OverviewPlot(data_store,ParameterSet({'sheet_name' : 'X_OFF', 'neuron' : sorted(param_filter_query(data_store,sheet_name="X_OFF").get_segments()[0].get_stored_esyn_ids())[0], 'sheet_activity' : {}}),fig_param={'dpi' : 50,'figsize': (28,24)},plot_file_name="LGN0Off.png").plot()
        OverviewPlot(data_store,ParameterSet({'sheet_name' : 'X_ON', 'neuron' : sorted(param_filter_query(data_store,sheet_name="X_ON").get_segments()[0].get_stored_esyn_ids())[1], 'sheet_activity' : {}}),fig_param={'dpi' : 50,'figsize': (28,24)},plot_file_name="LGN1On.png").plot()
        OverviewPlot(data_store,ParameterSet({'sheet_name' : 'X_OFF', 'neuron' : sorted(param_filter_query(data_store,sheet_name="X_OFF").get_segments()[0].get_stored_esyn_ids())[1], 'sheet_activity' : {}}),fig_param={'dpi' : 50,'figsize': (28,24)},plot_file_name="LGN1Off.png").plot()
        
        
        #AnalogSignalListPlot(data_store, ParameterSet({'sheet_name': "V1_Exc_L4"})).plot()
        import pylab
        pylab.show()
