...
def perform_analysis_and_visualization(data_store):
    ...
    if True:  #ANALYSIS
        ...
        #TrialAveragedFiringRate(param_filter_query(data_store,sheet_name=['V1_Exc_L4','V1_Inh_L4']),ParameterSet({'stimulus_type':"FullfieldDriftingSinusoidalGrating"})).analyse()
    ...
    if True: # PLOTTING
        ...
        OverviewPlot(dsv,ParameterSet({'sheet_name' : 'X_ON', 'neuron' : param_filter_query(data_store,sheet_name="X_ON").get_segments()[0].get_stored_esyn_ids()[0], 'sheet_activity' : {}}),fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
        OverviewPlot(dsv,ParameterSet({'sheet_name' : 'X_OFF', 'neuron' : param_filter_query(data_store,sheet_name="X_OFF").get_segments()[0].get_stored_esyn_ids()[0], 'sheet_activity' : {}}),fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
        OverviewPlot(dsv,ParameterSet({'sheet_name' : 'X_ON', 'neuron' : param_filter_query(data_store,sheet_name="X_ON").get_segments()[0].get_stored_esyn_ids()[1], 'sheet_activity' : {}}),fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
        OverviewPlot(dsv,ParameterSet({'sheet_name' : 'X_OFF', 'neuron' : param_filter_query(data_store,sheet_name="X_OFF").get_segments()[0].get_stored_esyn_ids()[1], 'sheet_activity' : {}}),fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
