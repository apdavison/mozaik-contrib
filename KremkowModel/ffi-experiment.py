...
import matplotlib
import time
from mozaik.framework.experiment import MeasureOrientationTuningFullfield, \
...
if True:
    ...
    experiment_list = [
        ...
        MeasureOrientationTuningFullfield(jens_model,
                                          num_orientations=1,
                                          spatial_frequency=0.8,
                                          temporal_frequency=2,
                                          grating_duration=148*7,
                                          contrasts=[1.0],
                                          num_trials=3),
    ]
...
if (not MPI) or (mpi_comm.rank == MPI_ROOT):
    ...
    if True:  
        ...
        l4_exc_phase = data_store.get_analysis_result(identifier='PerNeuronValue',
                                                      value_name = 'LGNAfferentPhase',
                                                      sheet_name = 'V1_Exc_L4')
        l4_exc = numpy.argmin([circular_dist(o, 0, numpy.pi)
                               for (o,p) in zip(l4_exc_or[0].values[:20],
                                                l4_exc_phase[0].values[:20])])
        l4_inh_or = data_store.get_analysis_result(identifier='PerNeuronValue',
                                                   value_name = 'LGNAfferentOrientation',
                                                   sheet_name = 'V1_Inh_L4')
        l4_inh_phase = data_store.get_analysis_result(identifier='PerNeuronValue',
                                                      value_name = 'LGNAfferentPhase',
                                                      sheet_name = 'V1_Inh_L4')
        l4_inh = numpy.argmin([circular_dist(o, 0, numpy.pi)
                               for (o,p) in zip(l4_inh_or[0].values[:20],
                                                l4_inh_phase[0].values[:20])])
    ...
    GSTA(
        select_result_sheet_query(data_store,"V1_Exc_L4"),
        ParameterSet({'neurons' : [l4_exc], 'length' : 50.0 }),
        tags=['GSTA']).analyse()
    Precision(
        select_result_sheet_query(data_store,"V1_Exc_L4"),
        ParameterSet({'neurons' : [l4_exc], 'bin_length' : 10.0 })).analyse()
    ...
    TrialAveragedFiringRate(
        l4_exc_data,
        ParameterSet({'stimulus_type':"FullfieldDriftingSinusoidalGrating"})).analyse()
    TrialAveragedFiringRate(
        l4_inh_data,
        ParameterSet({'stimulus_type':"FullfieldDriftingSinusoidalGrating"})).analyse()
    ...
    OverviewPlot(
        data_store,
        ParameterSet({'sheet_name' : 'X_ON', 'neuron' : 0, 'sheet_activity' : activity_plot_param}),
        fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
    OverviewPlot(
        data_store,
        ParameterSet({'sheet_name' : 'X_OFF', 'neuron' : 0, 'sheet_activity' : {}}),
        fig_param={'dpi' : 100,'figsize': (14,12)}).plot()
    Figure2Gratings(
        data_store,
        ParameterSet({'sheet_name' : 'V1_Exc_L4', 'neuron' : l4_exc}),
        plot_file_name='FigureL4Gratings.png',
        fig_param={'dpi' : 100,'figsize': (18,12)}).plot()
    dsv = analysis_data_structure_stimulus_filter_query(
            data_store,
            'FullfieldDriftingSinusoidalGrating')
    dsv = analysis_data_structure_parameter_filter_query(
            dsv,
            analysis_algorithm='TrialAveragedFiringRate')
    PlotTuningCurve(
        dsv,
        ParameterSet({'parameter_name' : 'orientation', 'neuron': l4_exc, 'sheet_name' : 'V1_Exc_L4'})).plot()
    PlotTuningCurve(
        dsv,
        ParameterSet({'parameter_name' : 'orientation', 'neuron': l4_inh, 'sheet_name' : 'V1_Inh_L4'})).plot()
    ...
    PeriodicTuningCurvePreferenceAndSelectivity_VectorAverage(
        dsv,
        ParameterSet({'parameter_name': 'orientation'})).analyse()
    
    PerNeuronValuePlot(
        analysis_data_structure_parameter_filter_query(
            data_store,
    ...
    PerNeuronValuePlot(
        analysis_data_structure_parameter_filter_query(
            ...
            sheet_name='V1_Exc_L4'),
        ParameterSet({}),
        plot_file_name='ORPREFL4.png').plot()


    if True:
        l4_exc_measured_or = data_store.get_analysis_result(identifier='PerNeuronValue',
                                                            value_name='orientation preference',
                                                            sheet_name='V1_Exc_L4')
        l4_inh_measured_or = data_store.get_analysis_result(identifier='PerNeuronValue',
                                                            value_name='orientation preference',
                                                            sheet_name='V1_Inh_L4')
        pylab.figure()
        pylab.subplot(2, 1, 1)
